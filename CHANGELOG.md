...
- New skill **`attestation`** — WhatsApp Key Attestation cert chains
  generated by the attestation-server sidecar. Backward-compatible
  additive change: existing API is untouched, `1.4.x` code keeps working.

  ```python
  async with RegHelpClient(api_key) as client:
      task = await client.get_attestation_token(authkey="<base64-challenge>")
      result = await client.wait_for_result(task.id, service="attestation")
...
- `AttestationStatusResponse` Pydantic model exposing `authorization`,
  `sign`, `leafPrivateKeyB64`, `keyboxDeviceId`.

- `wait_for_result(..., service="attestation")` now polls attestation
  tasks in addition to push/email/integrity/recaptcha/turnstile/voip.
